...
class DB
{
	 ...
	 *	4. Execute Query.
	 *	5. On exception : display message
	...
	/**
	 * If the SQL query  contains a SELECT statement it returns an array
	 * containing all of the result set row
	 * If the SQL statement is a DELETE, INSERT, or UPDATE statement it returns the
	 ...
	 *	@return mixed
	 */
	public function runSQL($sql, $fetchmode = PDO::FETCH_ASSOC)
	{
		$this->sQuery = $this->pdo->prepare(trim($sql));
		$this->sQuery->execute();
		return $this->sQuery->fetchAll($fetchmode);
	}
